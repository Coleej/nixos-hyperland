...
if command -v wofi >/dev/null 2>&1; then
  choice=$(printf "%s\n" "+5%" "-5%" "25%" "50%" "75%" "100%" | wofi --dmenu -p "Brightness (${current}% )")
elif command -v rofi >/dev/null 2>&1; then
  choice=$(printf "%s\n" "+5%" "-5%" "25%" "50%" "75%" "100%" | rofi -dmenu -p "Brightness (${current}% )")
else
  command -v notify-send >/dev/null 2>&1 && notify-send "Brightness" "wofi or rofi required"
  exit 1
fi
